...
class Quote_controller extends DatabaseController {
	...
	{
		$quote_array = array();
		$quote_array[] = get_group_by_attribute($id_number, $id_type);
		return $quote_array;
	}
	...
	{
		$quote_array = array();
		$query = 'select * from quote where $attribute_type = $attribute';
		...
		if($result)
		{
			while ($row = mysqli_fetch_array($result, MYSQLI_ASSOC))
			{
				...
				$quote_array[] = new Quote($row['quote_id'], $row['date_posted'], $row['author'], 
				$row['quote_text']);
			}
	...
	public function save_new($quote)
	{
		$sucess = true;
