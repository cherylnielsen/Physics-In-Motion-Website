...
/***
($notice_id, $from_member_id, $from_first_name, $from_last_name, $from_member_type, 
$to_member_id, $date_sent, $notice_subject, $notice_text, 
$response_to_notice_id, $flag_for_review)
***/

class NoticeToMember_View_Controller extends DatabaseController {
		...
		if($db_result)
		{
			while ($row = mysqli_fetch_array($db_result, MYSQLI_ASSOC))
			{
				...
				$notice_view->initialize($row['notice_id'], $row['from_member_id'], 
						...
						$row['from_member_type'], $row['to_member_id'], 
	...
	public function updateAttribute($notice_view, $key)
	{
		...
		switch ($key)
		{
			...
			case 'from_member_type':
				...
				break;
			case 'to_member_id':
				$value = $notice_view->get_to_member_id();	
				$query = "update $table set to_member_id = '$value' where notice_id = '$notice_id'";
				break;
			case 'date_sent':
