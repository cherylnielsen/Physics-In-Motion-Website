...
class Member_Data_Utilities {
	...
	public function get_notices_sent_by_member($member_id, $mdb_control)
	{
		...
		for($i = 0; $i < $num; $i++)
		{
			...
			$sent_to_sections = $mdb_control->getController("notice_sent_to_section")->getByAttribute("notice_id", $notice_id);
			$notices[$i]->set_sent_to_sections($sent_to_sections);
			$sent_to_members = $mdb_control->getController("notice_sent_to_member")->getByAttribute("notice_id", $notice_id);
	...
	public function get_notices_received_by_section($section_list, $mdb_control)
	{
		...
		{
			$section_id = $section_list[$i]->get_section_id();
			$list_by_notice_id = $mdb_control->getController("notice_sent_to_section")->getByAttribute("to_section_id", $section_id);
	...
	public function get_notices_received_by_member($member_id, $mdb_control)
	{
		...
		$notice_controller = $mdb_control->getController("notice_sent_to_member");
		$num = count($section_list);
